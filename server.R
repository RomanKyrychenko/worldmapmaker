  ...
  df <- reactive({
    ...
    zubat <- function(df){
      ...
      ggplot(world.ggmap, aes(map_id = id,group=group)) +
        ...
        expand_limits(x = world.ggmap$long, y = world.ggmap$lat) + theme_void() + theme(
          legend.position = "none"
        ) + #coord_map(xlim = c(x1,x2),ylim = c(y1,y2),"gilbert") 
        #coord_map("ortho", orientation = c(y1,x1,0))
        coord_map("ortho",orientation = c(input$coordlat,input$coordlon,0),xlim=c(input$minlon,input$maxlon),ylim=c(input$minlat,input$maxlat))
    }
    zubat(df)
  })
  ...
  df2 <- reactive({
    ...
    zubat <- function(df){
      ...
      ggplot() +
        ...
        #coord_map("ortho", orientation = c(y1,x1,0))
        coord_map("gilbert",xlim=c(input$minlon,input$maxlon),ylim=c(input$minlat,input$maxlat))
    }
    zubat(df)
  })
  ...
  df3 <- reactive({
    ...
    zubat <- function(df){
      ...
      ggplot(world.ggmap, aes(map_id = id,group=group)) +
        ...
        #coord_map("ortho", orientation = c(y1,x1,0))
        coord_map(xlim=c(input$minlon,input$maxlon),ylim=c(input$minlat,input$maxlat))
    }
    zubat(df)
  })
